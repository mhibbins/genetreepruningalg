...
double optimizer::get_initial_guesses()
{
    double initial = _p_scorer->initial_guesses();

    int i = 0;
    double first_run = _p_scorer->calculate_score(&initial);
    while (std::isinf(first_run) && i < NUM_OPTIMIZER_INITIALIZATION_ATTEMPTS)
    {
        initial = _p_scorer->initial_guesses();
        first_run = _p_scorer->calculate_score(&initial);
        i++;
    }
...
void NelderMeadSimilarityCutoff::Run(FMinSearch* pfm, optimizer::result& r, double& initial)
{
    pfm->tolx = 1e-6;
    pfm->tolf = 1e-6;
    fminsearch_min(pfm, &initial, [this](FMinSearch* pfm) { return threshold_achieved_checking_similarity(pfm); });
    auto result = get_best_result(pfm);
...
public:
    void Run(FMinSearch *pfm, optimizer::result& r, double& initial) override
    {
        ...
        std::cout << "*****Threshold achieved, move to Phase 2*****";
...
public:
    ...
    void Run(FMinSearch *pfm, optimizer::result& r, double& initial) override
    {
        std::vector<optimizer::result> results(PHASED_OPTIMIZER_PHASE1_ATTEMPTS);
...
public:
    void Run(FMinSearch *pfm, optimizer::result& r, double& initial) override
    {
        ...
        int phase1_iters = pfm->iters;
        auto phase1_result = get_best_result(pfm);
        initial = phase1_result->values[0]; //is zero-indexing here OK? Fixes error for now 
...
{
    std::unique_ptr<OptimizerStrategy> strat(get_strategy(params));

    std::cout << "\nOptimizer strategy: " << strat->Description();
    std::cout << "Iterations: " << params.neldermead_iterations;
    std::cout << "Expansion: " << params.neldermead_expansion;
    std::cout << "Reflection: " << params.neldermead_reflection << "\n";
    std::cout << "Starting Search for Initial Parameter Values";
    ...
    fminsearch_set_equation(pfm, _p_scorer, 1);

    strat->Run(pfm, r, initial);
    r.duration = std::chrono::duration_cast<std::chrono::seconds>(clock::now() - before);
...
std::ostream& operator<<(std::ostream& ost, const optimizer::result& r)
{
    if (r.score == -log(0))
    {
        ost << "\nFailed to find reasonable starting values to initialize search." << std::endl;
    }
    else
    {
        ost << "\nCompleted " << r.num_iterations << " iterations" << std::endl;
        ost << "Time: " << std::chrono::duration_cast<std::chrono::hours>(r.duration).count() << "H";
        ost << " " << std::chrono::duration_cast<std::chrono::minutes>(r.duration).count() % 60 << "M";
        ost << " " << std::chrono::duration_cast<std::chrono::seconds>(r.duration).count() % 60 << "S" << std::endl;
        ost << "Best match" << (r.values.size() == 1 ? " is: " : "es are: ") << std::setw(15) << std::setprecision(14);
        for (size_t i = 0; i < r.values.size() - 1; ++i)
            ost << r.values[i] << ',';
        ost << r.values[r.values.size() - 1] << std::endl; 
        ost << "Final -lnL: " << r.score << std::endl;
    }
