...
do
	...
	Rscript simulate_test_traits_sptree_longtree.R 2>/dev/null | python3 make_pruningalg_input_longtree.py test_input_$i.txt

	#Run inference and parse out the results 
	echo "Doing species tree inference..."
	../../../src/test.exe test_input_$i.txt sptree | python3 parse_pruningalg_output.py sptree $i >> singletrait_sigma10_sptreesims_longtree_test_results.txt
